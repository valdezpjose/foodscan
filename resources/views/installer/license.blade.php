...
@section('container')
    ...
    <div class="mb-4">
        <label class="text-sm font-medium block mb-1.5 text-heading">
            ...
            <span class="text-[#E93C3C]">*</span>
            <span class="text-primary modal-show underline cursor-pointer">
                ({{ trans('installer.license.active_process') }})
            </span>
        </label>
        ...
        @error('license_key')
            <small class="block mt-2 text-sm font-medium text-[#E93C3C]">
                {{ $message }}
            </small>
        @enderror
        @error('global')
    ...
    {{-- Aquí añadimos el campo oculto con el dominio actual --}}
    <input type="hidden"
           name="domain"
